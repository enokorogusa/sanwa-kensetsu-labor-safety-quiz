...
ミチハロ君奈良観光中.png              … マスコット（正解時に表示）
回答ミチハロ君 (002).png              … マスコット（不正解時に表示）
3Dミチハロ君.png                     … マスコット（トップ画面で表示）
```
...
三和建設の公式マスコット「ミチハロくん」の画像を、状況に応じて3種類使い分けています。

| 使う場面 | ファイル |
|---|---|
| トップ画面の紹介 | `3Dミチハロ君.png` |
| クイズで正解したとき | `ミチハロ君奈良観光中.png` |
| クイズで不正解だったとき | `回答ミチハロ君 (002).png` |

**画像を差し替えたい場合：**

- トップ画面の画像は `index.html` 内の `<img src="3Dミチハロ君.png" ...>` の部分を新しいファイル名に書き換える。
- 正解／不正解の画像は `js/app.js` 冒頭付近にある次の2行のファイル名を書き換える。
  ```js
